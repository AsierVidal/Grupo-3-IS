...
def cargar_y_visualizar_datos():
    ...
    if archivo:
        ...
        # Configure the event to adjust the scroll area when the canvas size changes
        canvas_main.bind("<Configure>", lambda event, canvas=canvas_main: on_canvas_configure(canvas_main))
        ...
        root.bind("<Configure>", lambda event, canvas=canvas: on_canvas_configure1(canvas))
        ...
        canvas.bind("<Configure>", lambda event, canvas=canvas: on_canvas_configure(canvas_main))
    ...
    if cargar == None:
        ...
        if columnas_x_seleccionadas and columna_y_seleccionada:
            ...
            try:
                ...
                # Configurar el evento para ajustar el área de desplazamiento cuando cambie el tamaño del lienzo
                 # Configurar el evento para ajustar el área de desplazamiento cuando cambie el tamaño del lienzo
                main_frame.grid_rowconfigure(9, weight=1)  # Ajuste de fila para que el label se e
                # Graficar el modelo
                grafica_modelo()
                mostrar_interfaz_grafica()
                
                # Crear el label debajo de frame_x y frame_y
                frame_x_bottom = frame_x.winfo_y() + frame_x.winfo_height()
                frame_y_bottom = frame_y.winfo_y() + frame_y.winfo_height()
                label_resultados.grid(row=10, column=0, pady=(10, 0), sticky="nsew")
        ...
        if columnas_x_seleccionadas and columna_y_seleccionada:
            ...
            try:
                ...
                label_resultados.grid(row=9, column=0, pady=(10, 0), sticky="nsew")

                guardar_boton.grid(row=10, column=0, pady=(10, 0), sticky="nsew")
                # Crear cuadros de entrada para la predicción
                frame_prediccion = tk.Frame(main_frame)
                frame_prediccion.grid(row=12, column=0, pady=(10, 0), sticky="nsew")  # Ajusta la posición según tus necesidades
    ...
    try:
        ...
        if columnas_x_seleccionadas and columna_y_seleccionada:
            ...
            try:
                ...
                frame_prediccion = tk.Frame(main_frame)
                frame_prediccion.grid(row=12, column=0, pady=(10, 0), sticky="nsew")
...
def realizar_prediccion():
    ...
    variables_x_seleccionadas_label = tk.Label(main_frame, text=f"Variables X seleccionadas: {', '.join(columnas_x_seleccionadas)}")
    variables_x_seleccionadas_label.place(x=400, y=920)
    ...
    if predicciones is not None:
        ...
        tk.messagebox.showerror("Predicción", modelo_g.get_coly() + str(round(predicciones, 3)))
    ...
    if datos is not None:
        ...
        if columnas_x_seleccionadas and columna_y_seleccionada:
            ...
            if len(columnas_x_seleccionadas) < 3:
                ...
                canvas_modelo.draw()
                canvas_modelo.get_tk_widget().grid(row=11, column=0, pady=(10, 0), sticky="nsew")
...
cargar_regresion = tk.Button(main_frame, text='Hacer Regresión', command=creador_de_modelo)
...
guardar_boton = tk.Button(main_frame, text='Guardar Modelo', command=obtener_datos_adicionales)
